...
function DuelTracker(msg)
  ...
  if winner and loser then
    if winner == myName then
      ourDuelSeen = true
      duelsWon = duelsWon + 1
      UltraStatisticsCharacterStats:UpdateStat('duelsWon', duelsWon)
    elseif loser == myName then
      ourDuelSeen = true
      duelsLost = duelsLost + 1
      UltraStatisticsCharacterStats:UpdateStat('duelsLost', duelsLost)
    end
  ...
  if ourDuelSeen then
    ...
    if total > 0 then
      UltraStatisticsCharacterStats:UpdateStat('duelsWinPercent', (duelsWon / total) * 100)
      UltraStatisticsCharacterStats:UpdateStat('duelsTotal', total)
    end
  end
end
