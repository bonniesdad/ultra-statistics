...
local function SerializeStringHelper(
ch -- Used by SerializeValue for strings
)
...
local function DeserializeValue(iter, single, ctl, data)
  ...
  elseif ctl == '^F' then -- ^F<mantissa>^f<exponent>
    ...
    if ctl2 ~= '^f' then
      error(
        "Invalid serialized floating-point number, expected '^f', not '" .. tostring(ctl2) .. "'"
      )
    end
    ...
    if not (m and e) then
      error(
        "Invalid serialized floating-point number, expected mantissa and exponent, got '" .. tostring(
          m
        ) .. "' and '" .. tostring(e) .. "'"
      )
    end
