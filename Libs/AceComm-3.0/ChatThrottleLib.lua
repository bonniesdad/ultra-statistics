...
function Ring:Add(
obj -- Append at the "far end" of the ring (aka just before the current position)
)
  if self.pos then
...
-- v25.
local function Ring_Link(
self,
  other -- Move and append all contents of another ring to this ring
)
...
function ChatThrottleLib.Hook_SendChatMessage(text, chattype, language, destination, ...)
  if bMyTraffic then return end
  local self = ChatThrottleLib
  local size =
    strlen(tostring(text or '')) + strlen(tostring(destination or '')) + self.MSG_OVERHEAD
...
function ChatThrottleLib.OnUpdate(this, delay)
  ...
  if self.OnUpdateDelay < 0.08 then return end
  self.OnUpdateDelay = 0
...
prio,
  ...
  if callbackFn and type(callbackFn) ~= 'function' then
    error(
      'ChatThrottleLib:ChatMessage(): callbackFn: expected function, got ' .. type(callbackFn),
      2
    )
  end
...
local function SendAddonMessageInternal(
self,
  sendFunction,
  prio,
  prefix,
...
prio,
  ...
  elseif callbackFn and type(callbackFn) ~= 'function' then
    error(
      'ChatThrottleLib:SendAddonMessage(): callbackFn: expected function, got ' .. type(callbackFn),
      2
    )
...
prio,
  ...
  if not self or not prio or not prefix or not text or not chattype or not self.Prio[prio] then
    error(
      'Usage: ChatThrottleLib:SendAddonMessageLogged("{BULK||NORMAL||ALERT}", "prefix", "text", "chattype"[, "target"])',
      2
    )
  elseif callbackFn and type(callbackFn) ~= 'function' then
    error(
      'ChatThrottleLib:SendAddonMessageLogged(): callbackFn: expected function, got ' .. type(
        callbackFn
      ),
      2
    )
...
prio,
  ...
  if not self or not prio or not prefix or not text or not gameAccountID or not chattype or not self.Prio[prio] then
    error(
      'Usage: ChatThrottleLib:BNSendGameData("{BULK||NORMAL||ALERT}", "prefix", "text", "chattype", gameAccountID)',
      2
    )
  elseif callbackFn and type(callbackFn) ~= 'function' then
    error(
      'ChatThrottleLib:BNSendGameData(): callbackFn: expected function, got ' .. type(callbackFn),
      2
    )
  elseif #text > 255 then
